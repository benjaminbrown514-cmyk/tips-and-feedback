...
const submitDiagnostics = (submissionObj) => {
    const diagnostics = Object.values(submissionObj);
    diagnostics.forEach((submission) => {
      if (submission.length > 0) {
        postDiagnostic(diagnostics);
      }
    });
...
const getDiagnostics = () =>
  ...
  fetch('/api/diagnostics', {
    ...
    getDiagnostics().then((data) => data.forEach((diagnostic) => createCard(diagnostic)));
  ...
  fetch('/api/feedback', {
    ...
    getFeedback().then((data) => data.forEach((fb) => createCard(fb)));
